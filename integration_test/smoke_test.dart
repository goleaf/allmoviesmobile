...
void main() {
  ...
  testWidgets('Smoke: routes load, images render, modal opens', (tester) async {
    ...
    await tester.runAsync(() async {
      Navigator.of(
        tester.element(find.byType(MoviesScreen)),
      ).pushNamed(SeriesScreen.routeName);
    });
    await _pumpUntilFound(
      tester,
      find.byType(SeriesScreen),
      timeout: const Duration(seconds: 8),
    );
    ...
    await tester.runAsync(() async {
      Navigator.of(
        tester.element(find.byType(SeriesScreen)),
    ...
    await tester.runAsync(() async {
      ...
      ).pushNamed(CompaniesScreen.routeName);
    });
    await _pumpUntilFound(
      tester,
      find.byType(CompaniesScreen),
      timeout: const Duration(seconds: 8),
    );
    ...
    await tester.runAsync(() async {
      await showModalBottomSheet<void>(
        context: ctx,
        builder: (_) =>
            const SizedBox(height: 120, child: Center(child: Text('Modal OK'))),
      );
    });
    ...
    await _pumpUntilFound(
      tester,
      find.byType(MoviesFiltersScreen),
      timeout: const Duration(seconds: 8),
    );
    ...
    await tester.runAsync(() async {
      Navigator.of(
        tester.element(find.byType(MoviesFiltersScreen)),
      ).pushNamed(SeriesFiltersScreen.routeName);
    });
    await _pumpUntilFound(
      tester,
      find.byType(SeriesFiltersScreen),
      timeout: const Duration(seconds: 8),
    );
