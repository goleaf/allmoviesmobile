...
class FakeTmdbRepository extends TmdbRepository {
  ...
  List<Movie> _oneMovie(String title) => [Movie(id: 1, title: title, mediaType: 'movie')];

  @override
  Future<List<Movie>> fetchNowPlayingMovies({int page = 1, bool forceRefresh = false}) async => _oneMovie('now');

  @override
  Future<List<Movie>> fetchPopularMovies({int page = 1, bool forceRefresh = false}) async => _oneMovie('popular');

  @override
  Future<List<Movie>> fetchTopRatedMovies({int page = 1, bool forceRefresh = false}) async => _oneMovie('top');

  @override
  Future<List<Movie>> fetchUpcomingMovies({int page = 1}) async => _oneMovie('upcoming');
  ...
  Future<PaginatedResponse<Movie>> discoverMovies({int page = 1, DiscoverFilters? discoverFilters, Map<String, String>? filters, bool forceRefresh = false}) async {
    return PaginatedResponse<Movie>(page: 1, totalPages: 1, totalResults: 1, results: _oneMovie('discover'));
  }
  ...
  Future<PaginatedResponse<Company>> fetchCompanies({required String query, int page = 1, bool forceRefresh = false}) async {
    if (query.trim().isEmpty) {
      return const PaginatedResponse<Company>(page: 1, totalPages: 1, totalResults: 0, results: <Company>[]);
    }
    ...
    return PaginatedResponse<Company>(
      page: json['page'] is int ? json['page'] as int : int.tryParse('${json['page']}') ?? 1,
      totalPages: json['total_pages'] is int ? json['total_pages'] as int : int.tryParse('${json['total_pages']}') ?? 1,
      totalResults: json['total_results'] is int ? json['total_results'] as int : int.tryParse('${json['total_results']}') ?? results.length,
      results: results,
    );
  }
  ...
  Future<List<Person>> fetchTrendingPeople({String timeWindow = 'day'}) async {
    final json = await _readJsonFixture('trending_person_${timeWindow}_page1.json');
    final results = (json['results'] as List? ?? const [])
        .whereType<Map<String, dynamic>>()
        .map(Person.fromJson)
        .toList(growable: false);
